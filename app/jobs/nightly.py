...
Pasos 3 y 4 (docs/NOTIFICACIONES.md §5.2): el resumen a la empresa y el
despacho de correos. El 3 va DESPUÉS de `recompute_all_statuses` a propósito:
"entró en mora anoche" se lee del estado que el paso 1 acaba de persistir.
...
async def run(*, now: datetime | None = None, provider: EmailProvider | None = None) -> None:
    ...
    # Sin `now` inyectado, el despachador toma su propio reloj DESPUÉS del
    # paso 3: las entregas que ese paso acaba de crear nacen con
