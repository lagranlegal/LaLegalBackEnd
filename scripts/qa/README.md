...
| `verificar_cadenas.py` | **Vigila las invariantes de las cadenas de contratos** (el recargo, `00051`) **sobre los datos vivos**: (1) todo contrato con sucesor está en `superseded` —la columna es `parent_contract_id`, no `root_contract_id`, que es la raíz de la cadena—, (2) ningún `contract_item` en `transferred` cuelga de un contrato que no lo esté, y (3) ningún contrato **terminal** conserva `extension_ends_at`. Los estados terminales salen de `rules.TERMINAL_STATUSES`, **no de una lista escrita a mano** — escribirla a mano es exactamente el error de F21-10. Va por SQL directo (`DATABASE_URL`) en una transacción **`SET TRANSACTION READ ONLY`**, y no imprime ningún dato personal: solo id, número, empresa y estado. Sale con **0** sano, **1** si una invariante está rota y **2** si **no se pudo verificar** (conexión caída, credencial mala). Ese tercer código no es un lujo: sin él un parpadeo de red sale con el 1 de Python y el cron reporta «las cadenas están rotas» cuando nadie las tocó — y un guardián que da falsas alarmas se empieza a ignorar. Al revés vale igual: *no se pudo verificar* tampoco es *está sano*. Mismo criterio que `verificar_job_nocturno.py`. Sirve en un cron. `QA_DATABASE_URL` apunta a otra base (la local de tests) para ejercer la detección. | **F21-10** — los 4 contratos que el job nocturno resucitó (Empresa Demo Front Nº 1 y Nº 20, LA GRAN LEGAL Nº 28, ZZ QA Nº 9), y la prórroga viva del Nº 1. Reparados el 21/09/2026; el script sale en verde desde entonces |
