...
def test_cash_statement_now_has_a_running_balance(
    client: TestClient, accounts_tenant: dict
) -> None:
    """El extracto del cajón SÍ lleva saldo corriente, desde 00048.

    Este test afirmaba lo contrario, y su argumento era el mismo que estaba
    escrito en el servicio: "la base del cajón se vuelve a declarar en cada
    apertura y NO es un movimiento, así que acumular el histórico daría un
    número sin significado".

    Lo que faltaba no era una excepción para el efectivo: era que la base de
    apertura FUERA un movimiento. Desde que lo es, el acumulado del cajón
    significa exactamente lo que tiene que significar —cuánto debería haber
    ahí— y un extracto sin saldo corriente era justo lo que impedía ver de
    dónde salía un faltante.

    Que el efectivo además se verifique CONTANDO no lo contradice: contar es
    lo que produce el ajuste que reconcilia el saldo.
    """
    token = accounts_tenant["token"]
    caja = next(a for a in _accounts(client, token) if a["type"] == "cash")
    ...
    assert body["has_running_balance"] is True
    assert body["opening_balance"] is not None
    assert body["closing_balance"] is not None
    assert all(line["running_balance"] is not None for line in body["lines"])
