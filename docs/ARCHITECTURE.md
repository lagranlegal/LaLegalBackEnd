...
Se invoca con `python -m app.jobs.nightly` (usa `app/core/logging.py`, mismo formato JSON que la API). Decisión de infraestructura (elegida explícitamente sobre pg_cron): un **Fly Machine programada**, no un cron dentro de Postgres — así la lógica de negocio sigue viviendo en un solo lugar (Python, reusando `contracts.service`/`platform.service` tal cual, sin portarla a SQL/PL-pgSQL). Ver `fly.dev.toml`/`fly.prod.toml` (proceso `nightly`, comentado con el comando `fly machine run --schedule daily` para crear la máquina programada — una por ambiente) y `Dockerfile`.
