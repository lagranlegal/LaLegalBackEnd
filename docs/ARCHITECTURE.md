...
**Excepción consciente a la regla de capas:** los routers de `identity`, `platform`, `contracts` y `sales` importan `notifications.dispatcher`, no una `integration`. Ponerlo en `notifications.integration` cerraba un ciclo de imports (`notifications.integration → dispatcher → identity.integration → notifications.integration`), porque el despachador necesita a `identity` para pedir un enlace nuevo cuando reintenta una invitación.
