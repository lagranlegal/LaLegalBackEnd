...
- **Desplegar de verdad en Fly.io**: `Dockerfile`, `fly.dev.toml` y `fly.prod.toml` ya están en el repo (imagen, proceso `web`, plantilla del proceso `nightly` con el comando exacto para programarlo vía `fly machine run --schedule daily`). `prod` además necesita su propio proyecto Supabase (hoy solo existe el de `dev`) antes de poder desplegarse.
