...
| `PATCH` | `/api/v1/customers/{id}` | `customers.create` | Edición parcial (no hay `customers.edit` en el catálogo de permisos — usa el mismo que crear). No permite cambiar `doc_type`/`doc_number` (identidad del cliente). `email` se valida igual que al crear (F21-19): si solo se validara el alta, bastaría con crear el cliente sin correo y agregárselo después. |
